...
const RecentActivity = ({ isDarkMode }) => {
    ...
    return (
        <div className={`rounded-md border ${isDarkMode ? 'border-gray-700 bg-gray-800' : 'border-border bg-surface'
            }`}>
            {/* Header */}
            <div className={`p-4 border-b ${isDarkMode ? 'border-gray-700' : 'border-border'
                }`}>
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <select
                            ...
                            className={`px-3 py-1 rounded-md border text-sm ${isDarkMode
                ...
                {filteredActivities.length === 0 ? (
                    ...
                    <div className="divide-y divide-border">
                        {filteredActivities.map((activity) => (
                            <div
                                key={activity.id}
                                className={`p-4 border-l-4 transition-colors hover:bg-opacity-50 ${getPriorityBorder(activity.priority)
                                    } ${isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-primary/5'}`}
                            >
                                <div className="flex items-start space-x-3">
                                    <div className={`p-2 rounded-md ${activity.priority === 'critical' ? 'bg-error/10' :
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center justify-between mb-1">
                                            ...
                                            <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-text-secondary'
                                                }`}>
                                                {getTimeAgo(activity.timestamp)}
                                            </span>
                                        </div>

                                        <p className={`text-sm mb-2 ${isDarkMode ? 'text-gray-300' : 'text-text-secondary'
                                            }`}>
                                            {activity.details}
                                        </p>
                                        ...
                                        <div className="flex items-center space-x-4">
                                            ...
                                            <div className="flex items-center space-x-1">
                                                ...
                                                <span className={`text-xs font-data ${isDarkMode ? 'text-gray-400' : 'text-text-secondary'
                                                    }`}>
            ...
            {/* Footer */}
            <div className={`p-3 border-t text-center ${isDarkMode ? 'border-gray-700' : 'border-border'
                }`}>
