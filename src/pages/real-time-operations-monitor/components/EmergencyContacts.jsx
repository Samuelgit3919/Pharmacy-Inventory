                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    {emergencyContacts?.map((contact) => (
                        <button
                            ...
                            className={`p-4 rounded-md border transition-colors text-left ${contact?.available
                                ? isDarkMode
                            ...
                            <div className="flex flex-col space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center space-x-2">
                                        <div className={`w-2 h-2 rounded-full ${contact?.available ? 'bg-success' : 'bg-error'
                                            }`}></div>
                                        <Icon name="Phone" size={16} className="text-primary" />
                                    </div>
                                </div>
                                <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-text-secondary'
                                    }`}>
                                    {contact?.role}
                                </div>
                                <div className={`text-xs font-data ${isDarkMode ? 'text-gray-400' : 'text-text-secondary'
                                    }`}>
                                    {contact?.phone}
                                </div>
                            </div>
